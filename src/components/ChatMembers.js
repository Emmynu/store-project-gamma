      ...
      {(!text ? newChats : filteredChats).map(chat=>{
        return <section className="my-2.5">
              {chat[1]?.members?.sender?.id === id ?<Link to={`/chat/${chat[0]}/${chat[1]?.members?.receiver?.id}`}>
                <article className="flex items-center  my-2.5">
                  <img className="w-[30px] h-[30px] object-cover rounded-[50%]" src={chat[1]?.members?.receiver?.url}/>
                  <div>
              ...
              : <Link to={`/chat/${chat[0]}/${chat[1]?.members?.sender?.id}`}>
                  <article className="flex items-center  my-2.5">
                    <img className="w-[30px] h-[30px] object-cover rounded-[50%]" src={chat[1]?.members?.sender?.url}/>
                   <div>
